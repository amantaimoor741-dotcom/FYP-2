...
function GoogleButton({ onSuccess }: { onSuccess: (credential: string) => void }) {
  ...
  const renderGoogleButton = () => {
    try {
      if (btnRef.current && (window as any).google?.accounts?.id) {
  ...
  if (error || GOOGLE_CLIENT_ID === 'your_google_client_id_here') {
    return (
      <button type="button" onClick={() => alert('Set VITE_GOOGLE_CLIENT_ID in .env with your real Google Client ID from https://console.cloud.google.com/apis/credentials')}
        className="w-full flex items-center justify-center gap-3 py-4 rounded-xl border border-white/10 hover:bg-white/5 transition-all text-sm text-text-muted"
      >
  ...
  return <div ref={btnRef} className="w-full flex justify-center min-h-[48px]" />;
}
